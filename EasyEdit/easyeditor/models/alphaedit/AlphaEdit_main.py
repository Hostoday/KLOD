...
def execute_AlphaEdit(
    ...
    for i, layer in enumerate(hparams.layers):
        ...
        upd_matrix = torch.linalg.solve(
                ...
                + hparams.L2 * torch.eye(layer_ks.shape[0], dtype=work_dtype, device=target_device),
    ...
    for i, layer in enumerate(hparams.layers):
        layer_ks = compute_ks(model, tok, requests, hparams, layer, context_templates).T.to(
