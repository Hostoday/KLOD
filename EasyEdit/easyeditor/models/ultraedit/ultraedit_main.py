...
class UltraEditRewriteExecutor:
    ...
    def init_model(self, model, tok, params: UltraEditHyperParams):
        ...
        if params.model_parallel:
            self.alg.lifelong_normalizer.to(self.alg.stat_device)
        else:
            self.alg.to(resolve_device(params.device))
    ...
    def apply_to_model(
        ...
        **kwargs
        ...
        input_device = self.alg.stat_device
        ...
        for i in range(hparams.batch_size // hparams.batch_size_once):
            ...
            # Tokenize
            sent_tok = self.tokenizer(sentences, padding=True, return_tensors="pt").to(input_device)
            target_tok = self.tokenizer(targets, padding=True, return_tensors="pt").to(input_device)
