...
class ULTRAEDIT(EditableModel):
    ...
    def __init__(
        ...
        self.stat_device = resolve_device(self.config.device)
        self.config.device = str(self.stat_device)
    ...
    def edit_model(
        ...
        for module_name, param_shift in param_shifts.items():
            ...
            module.weight.data += param_shift.to(
    ...
    def cache(self, batch) -> Dict[int, Dict[int, Dict[str, torch.Tensor]]]:
        ...
        for idx, t in enumerate(batch):
            ...
            for module_idx, module_name in enumerate(self.config.inner_params):
                ...
                keys = tr[module_name].keys.to(device=self.stat_device, dtype=torch.float32)
                values_grad = tr[module_name].values_grad.to(device=self.stat_device, dtype=torch.float32)
    ...
    def predict_param_shifts(self, module_kv_map) -> Dict[str, torch.FloatTensor]:
        ...
        for module_idx, module_name in enumerate(self.config.inner_params):
            ...
            v_feature = torch.empty((0, shape[1]), device=self.stat_device)
            ...
            with torch.no_grad():
                mat = hidden_states.T @ hidden_states + torch.eye(shape[0], device=self.stat_device)
    ...
    def to(self, device):
        super().to(device)
        self.stat_device = resolve_device(device)
        self.config.device = str(self.stat_device)
