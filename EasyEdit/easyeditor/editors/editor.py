...
def resolve_model_torch_dtype(hparams: HyperParams) -> torch.dtype:
    wants_bf16 = bool(getattr(hparams, "bf16", False))
    wants_fp16 = bool(getattr(hparams, "fp16", False))

    if wants_bf16:
        if torch.cuda.is_available() and torch.cuda.is_bf16_supported():
            return torch.bfloat16
        LOG.warning(
            "bf16=True was requested, but the current CUDA setup does not report bf16 support. "
            "Falling back to fp16/float32."
        )
        if wants_fp16:
...
def get_primary_model_device(model) -> Optional[torch.device]:
    try:
...
class BaseEditor:
    ...
    def __init__(self, hparams: HyperParams):
        ...
        if type(self.model_name) is str:
            ...
            torch_dtype = resolve_model_torch_dtype(hparams)
        ...
        if hparams.model_parallel:
            primary_device = get_primary_model_device(self.model)
            if primary_device is not None:
                primary_device = str(primary_device)
                hparams.device = primary_device.split(":")[1] if primary_device.startswith("cuda:") else primary_device
