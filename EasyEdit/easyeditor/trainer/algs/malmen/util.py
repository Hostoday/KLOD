...
def _indices_on_device(indices: Tuple[torch.LongTensor, ...], device: torch.device) -> Tuple[torch.LongTensor, ...]:
    return tuple(index if index.device == device else index.to(device=device, non_blocking=True) for index in indices)
...
class Tracer:
    ...
    def __init__(
        ...
        def forward_hook(
            ...
            layer_indices = _indices_on_device(cache_indices, inputs[0].device)
            self.keys = inputs[0][layer_indices].detach()
        ...
        def backward_hook(
            ...
            grad_output = outputs_grad[0]
            layer_indices = _indices_on_device(cache_indices, grad_output.device)
            self.values_grad = grad_output[layer_indices].detach()
